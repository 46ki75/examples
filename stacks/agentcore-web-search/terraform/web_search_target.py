...
bundled ``aws`` CLI model expose the ``connector`` target type yet, so Terraform
shells out to this helper. It is run via ``uv run`` so it uses the workspace's
pinned boto3, which is new enough to know the ``connector`` shape.

Usage:
    uv run python web_search_target.py create --region us-east-1 --gateway-id <id>
    uv run python web_search_target.py delete --region us-east-1 --gateway-id <id>
"""
