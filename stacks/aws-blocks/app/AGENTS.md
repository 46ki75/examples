...
- **Tests:** `test/e2e.test.ts` — run with `pnpm run test:e2e`
...
2. Test with `pnpm run test:e2e` — starts a dev server automatically if one isn't running
3. For faster iteration: run `pnpm run dev &` in the background, then run `pnpm run test:e2e` repeatedly (reuses the running server)
...
- `pnpm run deploy` — full production deploy to AWS
