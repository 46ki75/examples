...
test.before(async () => {
  ...
  if (!await isServerRunning()) {
    server = spawn('pnpm', ['run', 'dev:server'], {
      cwd: process.cwd(),
...
// ─── Activity (AsyncJob queue) ─────────────────────────────────────────────────
// activityJob.submit() enqueues and returns immediately; the handler that
// writes the entry and publishes it runs asynchronously (via setTimeout in
// local dev, SQS + Lambda on AWS) — so entries need a short poll rather than
// being visible right after the call that queued them.

async function waitForActivity(predicate: (entries: Awaited<ReturnType<typeof api.listActivity>>) => boolean) {
  for (let i = 0; i < 20; i++) {
    const entries = await api.listActivity();
    if (predicate(entries)) return entries;
    await setTimeout(250);
  }
  throw new Error('Activity entry did not appear within 5s');
}

test('activity: creating a todo queues a "created" entry', async () => {
  const todo = await api.createTodo('Queued task');
  const entries = await waitForActivity(es => es.some(e => e.todoId === todo.todoId && e.action === 'created'));

  const entry = entries.find(e => e.todoId === todo.todoId)!;
  assert.strictEqual(entry.title, 'Queued task');
  assert.strictEqual(entry.action, 'created');

  await api.deleteTodo(todo.todoId);
});

test('activity: toggle and delete each queue their own entry', async () => {
  const todo = await api.createTodo('Toggle then delete');
  await waitForActivity(es => es.some(e => e.todoId === todo.todoId && e.action === 'created'));

  await api.toggleTodo(todo.todoId);
  await waitForActivity(es => es.some(e => e.todoId === todo.todoId && e.action === 'toggled'));

  await api.deleteTodo(todo.todoId);
  const entries = await waitForActivity(es => es.some(e => e.todoId === todo.todoId && e.action === 'deleted'));

  const actions = entries.filter(e => e.todoId === todo.todoId).map(e => e.action);
  assert.deepStrictEqual(actions.sort(), ['created', 'deleted', 'toggled']);
});

test('activity: listActivity is newest first', async () => {
  const entries = await api.listActivity();
  for (let i = 1; i < entries.length; i++) {
    assert.ok(entries[i - 1].at >= entries[i].at, 'entries should be sorted newest first');
  }
  assert.ok(entries.length <= 20);
});

// ─── Settings (KVStore) ─────────────────────────────────────────────────────
// Run before any updateSettings call in this file so the "defaults" check
// below is meaningful — settings are per-user (keyed by username), not
// per-test, so order matters within this shared test-user session.

test('settings: defaults before any update', async () => {
  const s = await api.getSettings();
  assert.strictEqual(s.defaultSort, null);
  assert.strictEqual(s.theme, 'light');
});

test('settings: updateSettings merges a partial patch and persists it', async () => {
  const updated = await api.updateSettings({ theme: 'dark' });
  assert.strictEqual(updated.theme, 'dark');
  assert.strictEqual(updated.defaultSort, null); // untouched field preserved

  const fetched = await api.getSettings();
  assert.strictEqual(fetched.theme, 'dark');
});

test('settings: a second patch merges without clobbering the first', async () => {
  const updated = await api.updateSettings({ defaultSort: 'priority' });
  assert.strictEqual(updated.defaultSort, 'priority');
  assert.strictEqual(updated.theme, 'dark'); // still dark from the previous test

  const fetched = await api.getSettings();
  assert.strictEqual(fetched.defaultSort, 'priority');
  assert.strictEqual(fetched.theme, 'dark');
});
...
// For a manual test: run `pnpm run dev`, open two browser tabs, and create
